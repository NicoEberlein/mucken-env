...
> [!NOTE]
> **Work in Progress:** The original game "Mucken" knows 3 game modes: Muck, Wenz, Geier. At the beginning of each game there is a bidding stage where the players decide on what game to play.
> This is not yet implemented. The game mode can be changed by setting the corresponding strategy in the reset-method.
...
The environment uses the PettingZoo AEC API.
...
The environment provides an `action_mask` in the observation. Agents *must* use this mask, as playing an invalid card leads to immediate termination of the episode and a negative reward.
...
##### hand_score
A normalized scalar which quantifies the strength of the cards on the agent's hands. Can be used as an orientation how "risky" the agent should play.
...
Normalized vector with the information on how many points the team of the agent already earned in the current round.
...
##### high_card_status
A vector with 12 values. The values represent the strongest cards in the game sorted by color in descending order: `4x Ober, 4x Unter, 4x Ace`.
When playing `Wenz` or `Geier` game mode, the last 4 indices stay at `0` because either Unter or Ober are irrelevant in this game.
